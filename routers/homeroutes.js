...
//for payment investment
exports.payinvent = async function (req, res) {




    var book1 = new investment({ projectid: req.params.id, userid: req.session.user._id, amount: (req.params.amount) / 100, query: 'dsdfd' });
    book1.save(function (err, rep) {
...
exports.postProject = function (req, res) {
        ...
        var filename = (new Date).valueOf() + '-' + req.files[0].originalname.replace(/\s/g, '');
        fs.rename(req.files[0].path, './public/uploads/' + filename, function (err) {
    
        });
            var video = (new Date).valueOf() + '-' + req.files[1].originalname.replace(/\s/g, '');
            fs.rename(req.files[1].path, './public/uploads/' + video, function (err) {
...
exports.getProject = async function (req, res) {
    var newpro = [];
    models.project.find(async function (err, response) {
        await response.forEach(async element => {
            var r = await models.investments.aggregate([
                { $match: { $and: [{ projectid: element.id }] } }, {
                    $group: {
                        _id: null,
                        total: {
                            $sum: "$amount"
